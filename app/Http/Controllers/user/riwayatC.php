...
use Validator;
class riwayatC extends Controller
{
  public function index(){
    $activeMenu="Riwayat";
    return view('user/riwayat_penjualan/index',compact('activeMenu'));
  }
  public function readPage($halaman,Request $req){
    $limit=10;
    ...
    ->whereRaw('penjualan.id_user=?',[$req->session()->get('id_user_laravel_crud')])
    ->limit($limit)
    ->offset($offset)
    ->get();
  ...
  public function delete($id_penjualan,Request $req){
    ...
    $simpan=Penjualan::whereRaw('id_penjualan=? AND id_user=?',[$id_penjualan,$req->session()->get('id_user_laravel_crud')])->delete();
    if($simpan){
